...
def process_user(user, lib_map, index):
    ...
    for cat, meta in lib_map.items():
        ...
        # Use default weights if specific category is missing from config
        ...
        try:
            session.post(f"{CONFIG['JELLYFIN_URL']}/Library/VirtualFolders", params={"name": final_name, "collectionType": meta["collection_type"], "paths": [str(out)], "refreshLibrary": "true"}, json={}, timeout=TIMEOUT)
